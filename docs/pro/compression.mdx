...
<Info>
  Prompt compression is a [GoModel Pro](/pro/overview) feature and requires a
  license with the `compression` entitlement.
</Info>
...
- **No prompt-cache degradation observed.** Because the previously sent
  prefix of a session is replayed byte-for-byte and only new messages are
  rewritten, provider prompt caches keep hitting across turns.
- **Real savings of 2%–20%**, depending on the use case. Agents that re-read
  files, replay tool output, or carry long histories sit at the top of that
  range; short, non-repetitive chat sits at the bottom, where the compressor
  simply forwards requests unchanged.
