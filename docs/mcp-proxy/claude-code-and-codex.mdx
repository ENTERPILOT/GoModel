...
| Last error says `no MCP endpoint at that path` (older releases: `session not found`) | The server's `url` includes its MCP endpoint path, for example `/http` for mcp-devtools or `/v2/mcp` for Firecrawl |
