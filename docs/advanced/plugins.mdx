...
a rule like `a => aa` is applied once, not on its own output).
...
`503` with code `feature_unavailable`, and the dashboard hides the Guardrails and
Plugins pages. Guardrails are plugin instances, so `GUARDRAILS_ENABLED=true` turns the
...
- **Guardrails page**: *Create Guardrail* lists every plugin that implements a
  ...
  references it.
- **Plugins page**: every loaded plugin type with its version, hooks, source
  (built-in, registered, or the `.so` path), and health. Guardrail plugins
  (those that may block, answer, or warn) carry a shield and are listed
  first; header or prompt modifiers and routing strategies are plugins too,
  but not guardrails. A plugin that failed to load is listed with its error.
  The page is hidden while the plugin system is off.
...
- Set `Guardrail: true` when the plugin polices traffic, that is, it may
  block, answer, or warn (`llm_judge`, `string_replace`). A plugin that only
  edits traffic (`header_edit`, `system_prompt`, `llm_based_altering`) or
  routes it leaves it false. The runtime ignores the flag; the dashboard
  marks guardrail plugins and their instances with a shield and lists them
  first.
