...
place request rewrites are recorded). When an instance edits the prompt, the
revision also stores the request body as it was forwarded after the whole
chain, under the same `LOGGING_LOG_BODIES` and `LOGGING_LOG_REVISION_BODIES`
gates as request rewrites. Response and stream decisions are logged with the
request id.
