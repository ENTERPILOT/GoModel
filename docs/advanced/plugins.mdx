...
place request rewrites are recorded). Every instance that edited the prompt is
recorded as a change with the request sizes before and after the chain; only
the last of them also stores the request body as it was forwarded after the
whole chain (earlier edits get no body copy of their own), under the same
