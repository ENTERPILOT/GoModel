...
- **Plugins list**: at the bottom of the Plugins & Guardrails page, every
  loaded plugin type with its version, hooks, source (built-in, registered,
  or the `.so` path), and health. Guardrail plugins (those that may block,
  answer, or warn) carry a shield and are listed first; header or prompt
  modifiers and routing strategies are plugins too, but not guardrails. A
