...
among the transform instances of a stream applies to all of them. `0`, the
...
| `Prompt` | `*Prompt` | the conversation (`Messages` with stable IDs and typed `Parts`: text, image, audio, file, tool call, tool result, reasoning, refusal, opaque), `Tools`, `Params`, raw body. Edit through `SetText`, `SetToolArguments`, `SetToolResult`, `Insert`, `Append`, `Remove`, `SetParam`; `TextTargets(roles...)` lists every text part, tool-result text included, and `SetTargetText` writes one back |
| `Response` | `*Completion` | the response, `nil` until the response phase: `Choices` with the same `Part` model, `Usage`, raw body. Edit through `SetText`, `ReplaceText`, `SetFinishReason`; `TextTargets` and `SetTargetText` work as on `Prompt` |
...
- Implement `pluginapi.HealthChecker` (`Health(ctx) error`) when an instance
  ...
  as the reason, and is logged once per transition. Health is informational:
