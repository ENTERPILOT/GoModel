...
| --- | --- | --- |
| **Built in** | Ships with GoModel: `system_prompt`, `llm_based_altering`, `string_replace`, `tag_replace`, `header_edit`, `llm_judge`, and the `cheapest_healthy` routing strategy | nothing to do |
...
Installations without a `config.yaml` can list the same entries through the
environment: `PLUGINS_LOAD` is a comma-separated list of `file` values, each
optionally `file=sha256` to pin the digest. Only the final `=` followed by
exactly 64 hexadecimal characters starts a digest pin; any other entry is a
file name. When non-empty, it replaces the
`plugins.load` list from `config.yaml` (env > config, like every other
...
- Set `Guardrail: true` when the plugin's instances are guardrails: policies
  ...
  `string_replace`, `tag_replace`, `header_edit`, `system_prompt`,
  `llm_based_altering`);
