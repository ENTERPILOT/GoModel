...
| Stream goes silent mid-flight | Closed with a `504` after `HTTP_STREAM_IDLE_TIMEOUT` (default 300s) | No | Same |
| Malformed chunk mid-stream | Treated as a truncation: relayed, then `stream_incomplete` | No | Same |

The wait for the *first* token never counts toward the idle timeout, so slow
reasoning models are unaffected. Details:
...
Both are exported over OpenTelemetry as `gomodel.client.empty_responses` with
the same `reason` attribute. See
...
both directions. You lose routing, failover, budgets, and logging until it is
