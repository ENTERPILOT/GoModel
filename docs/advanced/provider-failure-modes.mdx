...
reasoning model is safe until the provider sends something — but any byte
starts the clock, including a keep-alive comment. Once the first byte is out
the response is committed to `200`, so a stall from then on can only be
reported in-band, never as a status. Details:
