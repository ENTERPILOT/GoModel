...
| `timeout_ms` | No | Upper bound for each call of the guardrail. Default `0` (none). |
...
| **warn** | The unchanged response plus an `X-GoModel-Guardrail: warn; code=<code>` header; the detail is stored in the audit trail. |
