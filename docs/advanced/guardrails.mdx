...
| `{{gomodel.label.<key>}}` | Value of a `key=value` request label, empty when not set. |
...
| Key | Required | Default | Description |
| --- | -------- | ------- | ----------- |
| `roles` | No | `["system"]` | Prompt messages tags are expanded in: `system` (includes developer), `user`, `assistant`, `tool`. Adding `user` lets callers read request values such as labels and rewrites template text in their prompts. |
