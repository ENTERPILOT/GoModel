...
| `{{gomodel.label.<key>}}` | Request label `<key>`, empty when not set. |
...
| Key | Required | Default | Description |
| --- | -------- | ------- | ----------- |
| `roles` | No | `["system", "user"]` | Prompt messages tags are expanded in: `system` (includes developer), `user`, `assistant`, `tool`. |
| `timezone` | No | `UTC` | IANA timezone for the date and time tags, for example `Europe/Warsaw`. |
