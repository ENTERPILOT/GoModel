...
| `type` | Yes      | Definition type: `system_prompt` or `llm_based_altering`. |
...
<Note>
  Outbound request-header changes are configured as [Header Policies](/advanced/header-policies),
  not as message guardrails. Historical `header_modification` guardrail definitions remain readable
  for migration, but new definitions use the dedicated configuration and admin API.
</Note>
