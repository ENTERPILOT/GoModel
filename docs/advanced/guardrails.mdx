...
| `type` | Yes      | Definition type: `system_prompt`, `llm_based_altering`, or the outbound policy type `header_modification`. |
...
Despite its legacy definition type, this is an **outbound header policy**, not
a message guardrail. New workflow versions reference it in `header_policies`.
The gateway resolves an immutable header plan after workflow selection and
before cache lookup, then applies it while constructing the selected primary
provider attempt. Translated, passthrough, realtime websocket, and WebRTC
routes share the same protected-header enforcement.

Header policies do not leak into internal LLM/embedder calls or into a
different failover provider. Retries of the same primary provider reuse the
same resolved plan. The effective plan is part of exact and semantic cache
identity, including values copied with `from_header`.

#### Settings

| Field | Required | Description |
| ----- | -------- | ----------- |
| `methods` | No | HTTP methods to match, such as `POST`. Empty matches every method. |
| `endpoints` | No | Public request paths to match. A trailing `*` matches a prefix, such as `/p/anthropic/*`. |
...
final values after credential redaction. Redaction also recognizes custom
credential-like names such as `X-Session-Token` and `X-Custom-Auth`. When
header logging is disabled, only set header names are stored. Removed headers
always contain names only. A request with no
...
For API-authored workflows, attach the named definition explicitly:
...
```json
{
  "workflow_payload": {
    ...
    "header_policies": [{ "ref": "pin-anthropic-beta", "step": 10 }]
  }
}
```

Workflow versions created by the preview implementation may still contain a
`header_modification` reference in `guardrails`. They remain readable and are
migrated into the egress stage when compiled in memory.
