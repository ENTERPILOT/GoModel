...
appear side by side. The **Plugins** page in the sidebar shows every loaded
plugin type, its hooks, source, and health. Not every plugin is a guardrail:
a plugin declares `Guardrail: true` in its manifest when it polices traffic
(it may block, answer, or warn), and the dashboard marks those, and the
instances built from them, with a shield. A `header_edit` or `system_prompt`
instance is managed on the same page but only edits traffic.
