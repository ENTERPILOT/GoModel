...
YAML reads this as an empty `providers:` section plus three unrelated top-level
keys, not as three providers:
...
Strict parsing rejects it at startup:

```text
failed to parse config.yaml: line 2: field uranium-geryon-9b not found
```

Indent provider entries two spaces under `providers:`. Before strict parsing,
this booted with zero YAML providers and no error.
