...
are stored truncated and flagged `response_body_too_big_to_handle`. For edits
the request entry records the prompt, parameters, and upload metadata
(filename, content type, size) — the uploaded image bytes are never stored.
