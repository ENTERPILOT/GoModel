...
| `MASTER_KEY_DISABLED` | Turn off master key authentication, leaving [managed keys](/features/users) and authentication extensions as the only credentials | `false`                |
...
| `SERVER_TRUSTED_PROXIES` | Networks your own proxies sit on; enables header-based client addresses | _(empty)_ |
| `SERVER_CLIENT_IP_HEADER` | Header a trusted proxy passes the client address in | `X-Forwarded-For` |
| `SERVER_TRUSTED_HOPS` | Proxies between the client and the gateway; `0` scans the chain instead | `0` |
...
Every client address GoModel reports — audit entries, rate limit keys, logs —
is the address of the connection that reached the gateway, so behind a reverse
proxy they all show the proxy. Set `SERVER_TRUSTED_PROXIES` to the networks
your own proxies sit on and GoModel reads the forwarded client instead.
Nothing is trusted until you list it, not even loopback, and changing these
...
Entries are CIDR networks, bare addresses (a single host), or a preset:
...
Prefer the one subnet your proxy sits on. `private` trusts every private
client to set its own forwarded address, which is the wrong answer when your
clients are on private addresses too — a home lab whose clients live on
`192.168.1.0/24` and whose cluster runs on `10.42.0.0/16` should list
`10.42.0.0/16` alone.

By default GoModel reads `X-Forwarded-For`, where each hop appends the address
it saw, and records the nearest hop that is not one of your trusted networks:
the last address your own infrastructure wrote, and the furthest one a client
cannot overwrite. A request from outside the listed networks, a chain it cannot
parse, and a chain made entirely of trusted networks all keep the connection
address — in that last case every entry is a claim an internal client could
have invented.

Two settings change how the header is read:

- `SERVER_CLIENT_IP_HEADER` names a different header. Anything other than
  `X-Forwarded-For` carries a single address and is taken verbatim, which is
  the correct reading behind an edge that resolves the client itself:
  `SERVER_CLIENT_IP_HEADER=CF-Connecting-IP` behind Cloudflare, or `X-Real-IP`
  behind an nginx that sets it. RFC 7239 `Forwarded` is not supported.
- `SERVER_TRUSTED_HOPS` takes the `X-Forwarded-For` entry at a fixed depth
  instead of scanning the chain — `1` is the rightmost entry, `2` the one
  before it. Use it when your edge sits at a known depth but rotates through
  addresses you cannot enumerate. The connection must still come from a trusted
  network, and a chain shorter than the configured depth falls back to the
  connection address.

Both require `SERVER_TRUSTED_PROXIES`; on their own they are a startup error
...
The master key is a single global credential: whoever holds it is every user
of the gateway at once. Once managed gateway API keys are issued, set
`MASTER_KEY_DISABLED=true` (`server.master_key_disabled` in YAML) to retire it
and leave those keys — together with any authentication extension you run,
such as SSO — as the only way in. Per-key user paths, budgets, rate limits,
and audit trails then apply to every request, including the admin API.

```bash
MASTER_KEY_DISABLED=true
```

GoModel forgets any configured master key at startup, so nothing accepts or
derives from it: it fails against the gateway, against
[`GET /v1/auth/verify`](/advanced/api-endpoints#key-verification), and against
`/admin`. A key left in the environment or `config.yaml` is inert rather than
an error, which is what makes the switch safe to flip ahead of removing it.

Disabling is not the same as leaving `GOMODEL_MASTER_KEY` empty, which runs the
gateway unauthenticated and opens `/admin/*` so the dashboard can bootstrap its
first managed key. A disabled master key fails closed instead: every request
needs a credential, and the admin API stays behind one carrying dashboard
access. Issue that credential before flipping the switch — with no master key,
no managed key, and no authentication extension, the gateway rejects every
request and logs a warning at startup saying so.
...
Media files — the audio and images captured for the audit log when
`LOGGING_LOG_AUDIO_BODIES` or `LOGGING_LOG_IMAGE_BODIES` is on — are kept
outside the database. Their metadata lives in the storage backend above; only
the bytes go here, so they are never capped in size and the database stays
small. See [ADR-0013](https://github.com/ENTERPILOT/GoModel/blob/main/docs/adr/0013-media-storage.md).

| Variable             | Description                                                  | Default      |
| -------------------- | ------------------------------------------------------------ | ------------ |
| `MEDIA_STORAGE_TYPE` | Blob backend: `filesystem` or `memory` (lost on restart)     | `filesystem` |
| `MEDIA_STORAGE_PATH` | Directory for the filesystem backend                         | `data/media` |

The default path follows the SQLite rule: `./data/media` when a `./data`
directory exists next to the process, otherwise the OS per-user data
directory. In containers mount a volume there; with several replicas use a
shared mount (EFS, an S3 FUSE mount) so every instance can serve every file.
Files are retained as long as the audit entries that reference them
(`LOGGING_RETENTION_DAYS`) and removed by an hourly sweep.
...
Audit entries record the client address the gateway resolved. Behind a reverse
proxy that is the proxy until you list its network in
`SERVER_TRUSTED_PROXIES` — see [client addresses behind a
proxy](#client-addresses-behind-a-proxy).
...
| `HTTP_STREAM_IDLE_TIMEOUT`     | Longest silence in seconds a stream may keep once its first bytes arrived; `0` disables | `300` (5 min) |

A stream that goes quiet for longer than `HTTP_STREAM_IDLE_TIMEOUT` is closed
with a `504` provider error instead of hanging until `HTTP_TIMEOUT`. The wait
for the first token does not count, so slow reasoning models are unaffected.
Raise it only for a provider that legitimately pauses for minutes mid-stream.
...
upstream `/models` fails, returns nil, or returns an empty list. An
OpenAI-compatible provider without a `/models` endpoint (404 or 405) is served
from its configured list and reported healthy, which suits single-API servers
such as speech-to-text. Set
