...
upstream `/models` fails, returns nil, or returns an empty list. A provider
without a `/models` endpoint (404 or 405) is served from its configured list and
reported healthy, which suits single-API servers such as speech-to-text. Set
