---
title: "Configuration"
description: "Configure GoModel from the dashboard, environment variables, .env files, or YAML."
...
Provider credentials do not require configuration files or environment
variables. Add them from the dashboard's **Providers** page for the fastest
setup, or use environment variables and YAML when you want declarative
...
Optionally set these to automatically register providers from the environment.
You can instead manage the same credentials from the dashboard.
...
### Dashboard (recommended for getting started)

Start GoModel without provider configuration, then open
`/admin/dashboard/providers-config` and select **Add Provider**. Choose the
provider type, enter its credentials, and save it. GoModel registers the
provider immediately and persists it in the configured storage backend.

Providers declared through environment variables or `config.yaml` take
precedence over dashboard-managed providers with the same name and are
read-only in the dashboard.

### Optional auto-discovery from environment variables

GoModel checks for well-known API key environment variables and automatically
registers providers. This is useful for container secrets and other
