...
<Note>
  `LOGGING_LOG_AUDIO_BODIES` is independent of `LOGGING_LOG_BODIES` and controls
  audio endpoint payloads specifically. When enabled, `/v1/audio/speech` stores
  its text input and the generated audio (base64, capped at 10 MB) so the
  dashboard can play it back, and `/v1/audio/transcriptions` stores upload
  metadata (never the raw audio bytes). It defaults to `false` because audio
  payloads are large and grow the audit store quickly. When disabled, audio
  responses are recorded as a lightweight `{content_type, bytes}` placeholder.
</Note>
