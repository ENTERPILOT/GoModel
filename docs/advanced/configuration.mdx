...
<Note>
  ...
  compression) stores its rewritten body alongside the original client body in
  the audit entry, and the prompt guardrails that edited the prompt store one
  more copy in their shared revision: the body as forwarded after the whole
  guardrail chain.
  Disabling it keeps the revision metadata (rewriter name, byte sizes, tokens
