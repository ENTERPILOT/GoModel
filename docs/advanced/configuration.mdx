...
| `PID_FILE`           | Where the running gateway records its process id for `gomodel --reload` | `data/gomodel.pid` next to a `./data` directory, otherwise the per-user data directory |
