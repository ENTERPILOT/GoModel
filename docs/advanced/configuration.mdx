...
<Warning>
  **Upgrading a PostgreSQL deployment past v0.1.59.** The `workflow_versions`
  ...
  converts the column in place — no action needed, but the change is one-way.
  An older binary started against a converted database fails to boot, so roll
  back by restoring a dump taken before the upgrade rather than by swapping the
  binary. Workflow `created_at` values keep their instant but lose sub-second
  precision and are rendered in UTC. SQLite and MongoDB deployments are
  unaffected.
</Warning>
