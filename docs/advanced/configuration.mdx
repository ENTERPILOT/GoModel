...
providers that define a list. YAML `providers.<name>.models` provides the same
model-list input for named provider blocks.
...
models:
  ...
  # should hide any upstream models that are not explicitly listed.
...
<Note>
  ...
  the exposed inventory for that provider. For Oracle, see the [Oracle
