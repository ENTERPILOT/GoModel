...
| `MASTER_KEY_DISABLED` | Turn off master key authentication and accept [managed keys](/features/users) only | `false`                |
...
and make those keys the only way in — per-key user paths, budgets, rate
limits, and audit trails then apply to every request, including the admin API.
...
needs a credential, and the admin API stays behind a managed key with dashboard
access. Create that key before flipping the switch — with no master key and no
managed key, the gateway rejects every request and logs a warning at startup
saying so.
