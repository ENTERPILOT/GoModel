...
| Variable     | Description                                                                                                                                                                       | Default              |
| ------------ | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------- |
| `LOG_FORMAT` | `text` for colorized human output, `json` for structured logs. Unset auto-detects: text when stdout is a character device (a terminal, and also `/dev/null`), JSON for a file or a pipe. Force `json` in production / CloudWatch / Datadog / GCP setups. | _(auto-detect)_      |
