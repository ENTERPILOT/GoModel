---
title: "Audio API: Speech-to-Text and Text-to-Speech"
...
icon: "audio-lines"
keywords: ["audio API", "text-to-speech", "speech-to-text", "transcription", "TTS", "STT", "input_audio"]
---
...
`speed` — are forwarded to the provider. The response `Content-Type` is the one
the upstream returns, falling back to the type implied by `response_format` (for
example `wav` → `audio/wav`).

Synthesized audio is **relayed as the provider produces it** — see
[Streaming](#streaming).
...
Add `stream=true` to receive the transcript as it is produced — see
[Streaming](#streaming).

### Self-hosted speech-to-text servers

Many self-hosted STT servers serve `/v1/audio/transcriptions` but no `/v1/models`.
Register one as an `openai` provider and list the model it serves:
...
providers:
  local-stt:
    ...
    api_key: "unused"
    models:
      - id: "whisper-1" # the model name your server expects
        metadata:
          modes: [audio_transcription] # lists it as an audio model
```

When `/models` answers 404 or 405, GoModel serves the configured models and
reports the provider as healthy. Other `/models` failures still mark it as
degraded.

## Streaming

GoModel forwards an audio body to the client **as the provider produces it**, so
time-to-first-byte tracks the provider instead of the whole generation:

| Call | What streams |
| --- | --- |
| `POST /v1/audio/speech` | Always. Chunked audio in the requested `response_format`, or server-sent events when the forwarded `stream_format: "sse"` asks for them. |
| `POST /v1/audio/transcriptions` | With the forwarded `stream: true`, as `text/event-stream` (`transcript.text.delta` events closed by `transcript.text.done`). Without it, the complete transcript in one response. |

```bash Incremental transcript
curl -N https://your-gateway/v1/audio/transcriptions \
  -H "Authorization: Bearer $GOMODEL_KEY" \
  -F "file=@speech.wav" \
  -F "model=gpt-4o-transcribe" \
  -F "stream=true"
...
Usage and [cost tracking](#cost-tracking) are unchanged: a streamed transcript is
priced from the token counts its `transcript.text.done` event reports, and
streamed speech from the duration of the audio that passed through. Both are
measured as the response streams rather than from a buffered copy of it, so a
response of any length is billed in full.

<Note>
  A streamed response cannot be retried once its first bytes have reached the
  client, so — as with streamed chat completions — GoModel does not retry a
  failed upstream attempt on `/v1/audio/speech` or on a `stream: true`
  transcription. A plain transcription request keeps the usual retries.
</Note>

<Note>
  Streaming applies to the OpenAI-compatible upstreams. Providers whose native
  audio API GoModel translates (Xiaomi MiMo, Cohere, MiniMax, ElevenLabs) return
  a complete body, which GoModel forwards as one response.
</Note>
...
- **No retries for a streamed response** — see [Streaming](#streaming).
...
- **Body logging on, audio on** — `/v1/audio/speech` stores its text input and the
  generated audio so the **dashboard renders an inline player**, and
  `/v1/audio/transcriptions` stores the uploaded audio (also playable in the
  dashboard) alongside the upload metadata (filename, model, params).

The audio bytes go to [media storage](/advanced/configuration#media-storage),
not into the audit database: the entry carries a `media_id` and the dashboard
fetches the file through `GET /admin/media/{id}`. Nothing caps the size, and a
streamed response is stored as it is relayed. Files are kept as long as the
