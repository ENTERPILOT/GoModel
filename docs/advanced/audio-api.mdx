...
(default `false`), independent of `LOGGING_LOG_BODIES`:

- **Disabled (default)** — the audio response is recorded as a lightweight
  `{content_type, bytes}` placeholder; no audio bytes are stored.
- **Enabled** — `/v1/audio/speech` stores its text input and the generated audio
  (base64, capped at 10 MB) so the **dashboard renders an inline player**, and
  `/v1/audio/transcriptions` stores upload metadata (filename, model, params) but
  never the raw uploaded audio bytes.
