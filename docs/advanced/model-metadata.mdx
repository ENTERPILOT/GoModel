...
4. **Provider discovery** — some providers report capabilities in their own
   ...
   context length, OpenRouter's architecture modalities and context length,
   Chutes' context length, max output, capabilities, and pricing, Ollama's
   `/api/show` capabilities, and llama.cpp's context window and modalities (see
   [llama.cpp](/providers/llamacpp)). Models declared via configured model lists
   skip this step.
