...
so the public error type set stays closed. The `501` status and
`unsupported_response_operation` code identify the unsupported operation.

The HTTP status is `501 Not Implemented`.
