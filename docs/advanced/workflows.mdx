...
- `header_policies` are an egress stage independent of the message `guardrails` feature flag; their resolved plan participates in response-cache identity and applies only to the selected primary provider route
