...
- `header_policies` are an egress stage independent of the message `guardrails` feature flag; their resolved plan participates in response-cache identity and applies only to the selected primary provider route; see [Header Policies](/advanced/header-policies)
- `HEADER_POLICIES_ENABLED=false` is the global operational kill switch; workflow definitions remain stored and compilable while application is disabled
