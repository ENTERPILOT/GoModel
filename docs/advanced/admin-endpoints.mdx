...
Restrict a key to specific models with `allowed_models` on `POST /admin/auth-keys`
or `PUT /admin/auth-keys/<key-id>/allowed-models` (`{"allowed_models": ["anthropic/*", "openai/gpt-4o"]}`;
...
whose lists also apply. `effective_models` is what a request on that path may
actually call, evaluated through the same authorizer inference uses (so
model-side virtual-model policies count too); an empty list with
`restricted: true` means the node's lists intersect to nothing. `configured` is `false` for nodes implied only by keys
