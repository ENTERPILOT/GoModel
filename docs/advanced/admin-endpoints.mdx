...
| Method | Path | Description |
| ------ | ---- | ----------- |
| `GET` | `/admin/plugins` | Every loaded plugin type: manifest, hook kinds, config schema, source, health |
| `GET` | `/admin/guardrails/types` | Guardrail types with their editor schema, `phases`, `source`, and `mutates` |
| `GET` | `/admin/guardrails` | Guardrail instances with `fail_mode`, `timeout_ms`, `phases`, and `summary` |
| `PUT` | `/admin/guardrails` | Create or update one instance (accepts `fail_mode` and `timeout_ms`) |
...
and [Workflows](/advanced/workflows#guardrail-steps).
