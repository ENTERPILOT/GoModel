...
the endpoint's format with the code `stream_incomplete` (for example
`data: {"error":{"code":"stream_incomplete",...}}` on chat completions). This
covers malformed chunks, a connection dropped in the middle of a tool call,
and similar truncations, so a client never mistakes a partial answer for a
complete one. The request log records the stream as a `stream_error`.
