...
This page covers the knobs. For what GoModel does when a provider changes its
response shape, answers `200` with nothing usable, or dies mid-stream, see
[Provider failure modes](/advanced/provider-failure-modes).
...
| `slow_call_threshold` | `0` (off) | Answers slower than this count as breaker failures |
...
**Sustained latency** can open the circuit too. Set
`circuit_breaker.slow_call_threshold` (or
`CIRCUIT_BREAKER_SLOW_CALL_THRESHOLD`) and any call the provider answers
slower than that counts as a breaker failure, exactly like a `5xx`. The slow
answer is still returned to its caller; after `failure_threshold` consecutive
slow or failed calls the circuit opens, later requests fail fast with a
failover-eligible `503`, and a slow recovery probe reopens it. See
[Fail over from a slow provider](#fail-over-from-a-slow-provider).

The clock covers the whole call including retries. For streaming requests it
stops at the first data event, so the threshold bounds time to first token,
not the length of the stream.

It is off by default because normal latency depends on the workload: a
non-streaming reasoning call can legitimately take minutes. Pick a value well
above your slowest healthy request, and override it per provider when
workloads differ. The threshold never cancels a request — that is
`HTTP_TIMEOUT`'s job, and a call that hits it already counts as a failure.
...
| `CIRCUIT_BREAKER_SLOW_CALL_THRESHOLD` | duration | `0`   | Answers slower than this count as failures; `0` disables |
...
## Fail over from a slow provider

A provider that still answers, but takes minutes to do so, never produces the
errors that open the circuit. `slow_call_threshold` makes latency count:
...
providers:
  opencode:
    ...
    resilience:
      circuit_breaker:
        scope: model # one slow model must not pause the whole provider
        slow_call_threshold: 60s
        failure_threshold: 3
...
Once three calls in a row to `opencode/deepseek-v3` take longer than 60
seconds (or fail), its circuit opens and `resilient-chat` requests go straight
to `openai/gpt-4o-mini` without waiting. Every `timeout` (default 30s) one
probe request tries the primary again; traffic returns once probes answer
within the threshold.

A fast answer in between resets the count, so occasional slow calls never
open the circuit. Prefer `scope: model` here: with the default
`scope: provider`, fast models on the same provider keep resetting the count
for the slow one.
