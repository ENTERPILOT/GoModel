...
the credential editor ("Quota breaker"). Providers declared in `config.yaml`
or environment variables show their rules read-only there; change them in
the YAML file. Trip rules have no environment-variable equivalent.
