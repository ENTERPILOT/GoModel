...
| ------------------------ | -------- | ------- | ------------------------------------------ |
| `GOMODEL_RETRY_MAX_RETRIES`      | int      | `3`     | Maximum retry attempts per request         |
| `GOMODEL_RETRY_INITIAL_BACKOFF`  | duration | `1s`    | First retry wait (e.g. `500ms`, `2s`)      |
| `GOMODEL_RETRY_MAX_BACKOFF`      | duration | `30s`   | Upper cap on retry wait                    |
| `GOMODEL_RETRY_BACKOFF_FACTOR`   | float    | `2.0`   | Exponential multiplier between retries     |
| `GOMODEL_RETRY_JITTER_FACTOR`    | float    | `0.1`   | Random jitter as a fraction of the backoff |

### Circuit Breaker

| Variable                            | Type     | Default | Description                                   |
| ----------------------------------- | -------- | ------- | --------------------------------------------- |
| `GOMODEL_CIRCUIT_BREAKER_FAILURE_THRESHOLD` | int      | `5`     | Consecutive failures before opening           |
| `GOMODEL_CIRCUIT_BREAKER_SUCCESS_THRESHOLD` | int      | `2`     | Consecutive successes to close again          |
| `GOMODEL_CIRCUIT_BREAKER_TIMEOUT`           | duration | `30s`   | How long the circuit stays open before probing |
...
<Note>
  ...
  global defaults only — `GOMODEL_RETRY_MAX_RETRIES` cannot target a single provider.
...
- at startup,
- on every model registry refresh, controlled by `GOMODEL_CACHE_REFRESH_INTERVAL`
  (seconds, default `3600` — hourly),
- on the fast recheck loop, which re-probes **only** providers whose latest
  refresh failed, controlled by `GOMODEL_PROVIDER_RECHECK_INTERVAL`
...
`GOMODEL_PROVIDER_RECHECK_INTERVAL` seconds, updating "Last checked" and restoring
