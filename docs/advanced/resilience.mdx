...
The same rules are settable without a config file. Globally:

```bash
# semicolon-separated `pattern=ttl` pairs; ttl is an optional Go duration
CIRCUIT_BREAKER_TRIP_ON="weekly usage limit=4h;quota exceeded=15m"
```

Per provider — the value **replaces** the global rules for that provider only:

```bash
KIMICODE_TRIP_ON="weekly \(7-day\) usage limit=4h"
```

A malformed entry (empty pattern or invalid duration) fails config load,
