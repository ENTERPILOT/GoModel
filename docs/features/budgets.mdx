...
```env
GOMODEL_SET_BUDGET_TEAM__ALPHA="daily=10,weekly=50"
GOMODEL_SET_BUDGET_="monthly=500"
```

The suffix after `GOMODEL_SET_BUDGET_` becomes a user path. Use double underscores
(`__`) between path segments; single underscores remain part of a segment:

- `GOMODEL_SET_BUDGET_TEAM__ALPHA` -> `/team/alpha`
- `GOMODEL_SET_BUDGET_USER_123` -> `/user_123`
- `GOMODEL_SET_BUDGET_` -> `/`
