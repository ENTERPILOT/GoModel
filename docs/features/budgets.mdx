...
Spend counts toward a budget once the usage logger writes it (every
`USAGE_FLUSH_INTERVAL`, 5 seconds by default), so requests sent in that gap can
overshoot a limit slightly. With several GoModel instances, spend written by
another instance can take up to 2 more seconds to be seen.
