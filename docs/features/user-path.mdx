...
Global credentials see every object, including legacy rows recorded before the
path was stored. Objects the gateway has no record of (an ID stored only by the
upstream provider) are still looked up at the provider as before; use a
persistent store so every object stays tracked.
