...
| SSO or other extension identity         | Global, unless the extension sets an access scope |

An extension identity (for example a GoModel Pro SSO session) carries a
personal user path such as `/users/alice` for attribution. That path never
scopes the identity; an SSO admin keeps the whole gateway unless the extension
sets an explicit access scope for the session.
