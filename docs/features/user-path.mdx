...
the key then follows the user's current derived path even after the user or
its group moves in the tree. See [Users & Groups](/features/users-groups).
...
Policies can also use `groups` to allow a registered group and everything
beneath it in the group tree. See [Users & Groups](/features/users-groups)
for the registry and the per-user / per-group access editor.
