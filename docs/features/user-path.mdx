---
title: "User Path"
description: "Use user_path to scope keys, model access, model lists, workflows, usage, and audit views."
icon: "route"
---
...
When `MODEL_OVERRIDES_ENABLED=true`, model access overrides can use `user_paths`
to limit a selector to a subtree.
...
## Filter exposed models

When model overrides are enabled, `GET /v1/models` uses the effective
`user_path` too.
