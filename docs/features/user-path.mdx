...
A key can also bind to a registered user (`user_id`) instead of a raw path;
the key then follows the user's current path even after the user moves in the
hierarchy. See [Users & Groups](/features/users-groups).
...
Policies can also use `groups` to allow members of a named user group,
regardless of where their paths live in the hierarchy. See
[Users & Groups](/features/users-groups) for the registry and the per-user /
per-group access editor.
