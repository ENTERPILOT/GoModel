...
API keys additionally carry a per-key allowlist. The master key has no user
path and is never restricted.
...
| `*`                | everything (useful only to override a stricter parent — it cannot; see rule 3) |
