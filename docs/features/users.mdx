...
what a request on that path can actually call once every parent's list is
applied (a red **None** flags a node whose lists intersect to nothing) — and
how many API keys are bound to it. The editor warns while you type when the
