...
handshake with HTTP 404 or 405: something listens there, but not an MCP
endpoint at that path. Servers mount their endpoint under different paths, so
`url` must include it — for example `http://localhost:18080/http` for mcp-devtools or
`http://localhost:3000/v2/mcp` for self-hosted Firecrawl. Stateless servers
(no `Mcp-Session-Id`) need no extra configuration.
