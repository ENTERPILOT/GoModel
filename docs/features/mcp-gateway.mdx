...
- **Observability.** Every tool call becomes a usage entry (server, tool,
  ...
  the audit log and request log like any other model traffic. User-path rate
  limits and budgets gate MCP requests too.
...
or **disabled**, with tool counts and the last error. A server whose listing
fails keeps its previous catalog (marked degraded) and is re-probed every 60
seconds; healthy catalogs re-list every 5 minutes and refresh immediately when
the upstream sends a `list_changed` notification. **Reconnect** on the
dashboard (or `POST /admin/mcp-servers/{name}/reconnect`) forces a redial.
