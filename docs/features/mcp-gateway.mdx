...
them. Visibility is also checked on every call, so narrowing either list
takes effect for MCP sessions that are already open. In the dashboard, both
lists are under **Advanced** in the server editor.
