...
- **Observability.** Every tool call becomes a usage entry (server, tool,
  ...
  `GOMODEL_LOGGING_LOG_BODIES=true` the JSON-RPC request and response frames are
...
```yaml
mcp:
  enabled: true # default; GOMODEL_MCP_ENABLED=false disables the endpoints
  servers:
    github:
...
Or via the `GOMODEL_MCP_SERVERS` env var (a JSON object, merged over YAML per name):
...
- **stdio is declarative-only.** stdio servers spawn subprocesses on the
  gateway host, so they can only be declared in `config.yaml` / `GOMODEL_MCP_SERVERS`.
...
- **Dashboard-managed credentials live in the gateway database.** Headers of
  ...
  `GOMODEL_MCP_SERVERS` instead — `${ENV}` references keep the secret in the
