...
- Dashboard-managed mappings become virtual models with the failover strategy
  ...
  removes the old `failover_rules` store. A mapping whose primary model already
  had a virtual model is logged and skipped — add its fallbacks as targets of
  that virtual model instead.
