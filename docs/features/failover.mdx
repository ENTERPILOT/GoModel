...
- Dashboard-managed mappings become virtual models with the failover strategy
  ...
  then removes the old `failover_rules` store. A mapping whose primary model
  already had a virtual model is logged and skipped, and the store is kept
  until it is resolved — add its fallbacks as targets of that virtual model,
  then delete the row.
- `failover.rules`, `manual_rules_path`, `FAILOVER_RULES_JSON`, and
  ...
  virtual models on every start, with a deprecation warning. Move them under
  `virtual_models` at your convenience.
