...
Failover reacts to failed attempts, not slow ones. To move traffic off a
provider that still answers but takes too long, set a
[slow-call threshold](/advanced/resilience#fail-over-from-a-slow-provider) on
its circuit breaker.
...
Streaming requests are checked the same way before any bytes reach the client.
A `200` stream that ends without an event is treated as a `502`
(`provider returned an empty stream`), and a stream whose first event is an
`{"error": ...}` payload fails with the status that payload carries (for
example an in-band `429`), or `502` when it carries none. Both fail over under
the default rules. The response headers wait for that first event.

[Provider failure modes](/advanced/provider-failure-modes) lists every such
case and which of them fail over.
