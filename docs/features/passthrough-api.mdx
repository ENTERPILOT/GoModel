...
- `openai`, `anthropic`, `openrouter`, `kilo`, `zai`, `vllm`, `llmd`, and `deepseek` are enabled by
  default.
...
ENABLED_PASSTHROUGH_PROVIDERS=openai,anthropic,openrouter,kilo,zai,vllm,llmd,deepseek
```
