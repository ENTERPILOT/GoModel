...
- `openai`, `anthropic`, `chutes`, `openrouter`, `kilo`, `zai`, `sglang`, `vllm`, `llmd`, and `deepseek` are enabled by
  default.
...
ENABLED_PASSTHROUGH_PROVIDERS=openai,anthropic,chutes,openrouter,kilo,zai,sglang,vllm,llmd,deepseek
```
