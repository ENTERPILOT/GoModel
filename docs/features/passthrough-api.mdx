...
`GOMODEL_ALLOW_PASSTHROUGH_V1_ALIAS=true`, which is the default:
...
```env
GOMODEL_ENABLE_PASSTHROUGH_ROUTES=true
GOMODEL_ALLOW_PASSTHROUGH_V1_ALIAS=true
GOMODEL_ENABLED_PASSTHROUGH_PROVIDERS=openai,anthropic,openrouter,kilo,zai,vllm,deepseek
```

Set `GOMODEL_ENABLED_PASSTHROUGH_PROVIDERS` to the provider types you want to expose.
