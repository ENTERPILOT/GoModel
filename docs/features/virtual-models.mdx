...
models in `config.yaml` or the `GOMODEL_VIRTUAL_MODELS` environment variable instead of
...
```env
GOMODEL_VIRTUAL_MODELS=[{"source":"smart","strategy":"cost","targets":[{"model":"openai/gpt-4o"},{"model":"groq/llama-3.3-70b"}]}]
```
...
```env
GOMODEL_KEEP_ONLY_ALIASES_AT_MODELS_ENDPOINT=true
```
