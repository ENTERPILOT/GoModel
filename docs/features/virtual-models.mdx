...
For streamed responses — Chat Completions and Responses API SSE, and the
[audio endpoints](/advanced/audio-api#streaming) — GoModel drains the upstream in
the background and releases each read chunk on the scaled timeline. This
preserves the provider's relative chunk timing while delayed chunks accumulate
in memory. A large factor on a long or high-volume stream, or on a large audio
response, can therefore use substantial memory; leave slowdown unset in
production, where it has no purpose. Realtime WebSocket sessions are not slowed.
