...
inference time. The accepted range is `0.1` to `10`; for example, `0.5` adds
50%, so a response that takes 2 seconds upstream is returned after about 3
seconds. Leaving the field empty disables slowdown. Delays honor request
cancellation.
...
slowdown. This also applies to load-balanced virtual models after a target is
selected.
...
`slowdown` (an extra-time factor from `0.1` to `10`), and `enabled`.
