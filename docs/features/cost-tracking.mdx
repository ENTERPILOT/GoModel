...
- **Image tokens** from token-billed image models (the `gpt-image` family) at the model's `input_image_per_mtok` and `output_image_per_mtok` rates — every output token of an [image generation](/advanced/images-api) is an image token, and prompt image tokens carry their own input rate. Models without image rates fall back to the plain token rates.
