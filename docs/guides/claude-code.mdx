...
<Note>
  ...
  `ANTHROPIC_BASE_URL` and `ANTHROPIC_AUTH_TOKEN`. GoModel still needs an
  Anthropic API key to reach Anthropic upstream, but that key can be stored in
  the dashboard rather than an environment variable.
</Note>
...
3. Copy the key once. You will add it to GoModel from the dashboard.
...
authenticate with `change-me`, select **Add Provider**, choose `anthropic`, and
enter the API key. The provider becomes available immediately.
