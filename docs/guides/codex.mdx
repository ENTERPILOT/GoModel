...
- Have credentials ready for an upstream provider with the models you want to use.
...
<Note>
  ...
  its own. The upstream key can be stored in the GoModel dashboard. Later in
  this guide, `OPENAI_API_KEY=change-me` is the GoModel master key that Codex
  sends to GoModel, not your OpenAI Platform key.
</Note>

## 1. Run GoModel

Start GoModel with a master key:
...
Open [the Providers page](http://localhost:8080/admin/dashboard/providers-config),
authenticate with `change-me`, select **Add Provider**, and enter the upstream
credentials. The examples below use OpenAI, but the GoModel Responses API can
also translate requests for other supported providers.
...
This step is optional. If you are sure you have configured a working provider
in GoModel, you can skip it and go straight to
