...
<Note>
  The first path segment is your **provider instance name** in config (e.g. `/p/vllm/...`
  only if the YAML key is `vllm`). For multiple vLLM backends, add separate keys
  under `providers:` and call the matching passthrough base for each (e.g.
  `/p/my-vllm/tokenize` when the key is `my-vllm`). For OpenAI-style
  routing with a single entrypoint, you can also use translated `/v1/...` requests
  with provider-qualified model IDs such as
  `vllm-test/meta-llama/Llama-3.1-8B-Instruct`.
</Note>
