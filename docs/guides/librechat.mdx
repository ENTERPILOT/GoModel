...
| `ALLOW_REGISTRATION` | Without a mounted `.env`, LibreChat refuses new sign-ups by default. Turn it off again after your users have accounts. |
| `SEARCH` | Disables Meilisearch chat search so no extra container is needed. |
| `JWT_SECRET`, `CREDS_KEY`, ... | LibreChat's session and encryption secrets. Generate real values with `openssl rand -hex 32` (and `-hex 16` for `CREDS_IV`). |

Open `http://localhost:3080`, register an account, and the `GoModel`
endpoint appears in the model picker with every model GoModel exposes —
