...
GoModel exports OpenTelemetry traces and metrics for every inbound HTTP request
and every outbound model provider call. Provider calls follow the
...
`OTEL_ENABLED` is the only GoModel-specific switch. It can also be set as
`opentelemetry.enabled: true` in [`config.yaml`](/advanced/config-yaml).
Everything else follows the standard
[OpenTelemetry environment variables](https://opentelemetry.io/docs/specs/otel/configuration/sdk-environment-variables/),
so GoModel is configured exactly like any other instrumented service.

| Variable | Default | Purpose |
| --- | --- | --- |
| `OTEL_ENABLED` | `false` | Turn export on. |
| `OTEL_SERVICE_NAME` | `gomodel` | `service.name` resource attribute. |
| `OTEL_RESOURCE_ATTRIBUTES` | — | Extra resource attributes, e.g. `deployment.environment=prod,service.namespace=ai`. |
| `OTEL_EXPORTER_OTLP_ENDPOINT` | `http://localhost:4318` | Collector endpoint. `OTEL_EXPORTER_OTLP_TRACES_ENDPOINT` and `OTEL_EXPORTER_OTLP_METRICS_ENDPOINT` override it per signal. |
| `OTEL_EXPORTER_OTLP_PROTOCOL` | `http/protobuf` | `http/protobuf` or `grpc`; overridable per signal with `OTEL_EXPORTER_OTLP_TRACES_PROTOCOL` and `OTEL_EXPORTER_OTLP_METRICS_PROTOCOL`. |
| `OTEL_EXPORTER_OTLP_HEADERS` | — | Headers such as `authorization=Bearer ...` for hosted backends. |
| `OTEL_TRACES_EXPORTER` | `otlp` | `otlp` or `none` to switch traces off. |
| `OTEL_METRICS_EXPORTER` | `otlp` | `otlp` or `none` to switch metrics off. |
| `OTEL_TRACES_SAMPLER` | `parentbased_always_on` | Sampler, e.g. `parentbased_traceidratio` with `OTEL_TRACES_SAMPLER_ARG=0.1` to keep 10% of new traces. |
| `OTEL_PROPAGATORS` | `tracecontext,baggage` | Comma-separated: `tracecontext`, `baggage`, `b3`, `b3multi`, `jaeger`, `ottrace`, or `none`. |
...
| `error.type` | `429`, `timeout`, `network_error` | Present only on failures. |
...
| Metric | Recorded for |
| --- | --- |
| `gen_ai.client.operation.duration` | Every buffered call, and every streaming call that fails before the stream is established. |
...
establish gets a retrospective failure span, so errors are always traced.
