...
`text.verbosity` through), while returning a clear error for stateful or
provider-native Responses fields such as `previous_response_id`, `conversation`,
and hosted tool items.
