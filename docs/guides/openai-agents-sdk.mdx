...
that does not implement Responses natively, GoModel returns a clear error for
stateful or provider-native Responses fields such as `previous_response_id`,
`conversation`, hosted tool items, and structured `text` output settings.
...
- Hosted tools such as web search, file search, and computer use require a
  native upstream provider that supports those tool payloads.
