...
- The `endpoint` label is a path template: the query string is dropped and
  resource IDs become `{id}` (for example `/files/{id}/content`), so file,
  batch, and response IDs do not create new series. At most 256 distinct
  endpoint values are tracked; further paths (for example, unusual passthrough
  paths) are reported as `/{other}`
