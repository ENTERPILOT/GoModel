...
`GOMODEL_METRICS_ENABLED=true` and start GoModel:

<CodeGroup>

```bash Docker (.env file)
# Add GOMODEL_METRICS_ENABLED=true to your .env file, then:
...
docker run --rm -p 8080:8080 \
  -e GOMODEL_METRICS_ENABLED=true \
  enterpilot/gomodel
# Metrics available at http://localhost:8080/metrics
```
...
export GOMODEL_METRICS_ENABLED=true
./bin/gomodel
# Metrics available at http://localhost:8080/metrics
```
...
```bash
GOMODEL_METRICS_ENABLED=false
```

**Option 2: .env file**

```bash
echo "GOMODEL_METRICS_ENABLED=false" >> .env
```
...
GOMODEL_METRICS_ENDPOINT=/internal/prometheus
```
...
| ------------------ | ---------- | --------------------------------- |
| `GOMODEL_METRICS_ENABLED`  | `false`    | Enable/disable metrics collection |
| `GOMODEL_METRICS_ENDPOINT` | `/metrics` | HTTP path for metrics endpoint    |
...
GOMODEL_METRICS_ENABLED=true
GOMODEL_METRICS_ENDPOINT=/metrics
OPENAI_API_KEY=sk-...
```
...
Set `GOMODEL_METRICS_ENABLED=true` via your `.env` file, an inline `-e GOMODEL_METRICS_ENABLED=true`
flag, or `export GOMODEL_METRICS_ENABLED=true` (see the [Quick Start](#disabled-by-default)
...
```bash
export GOMODEL_METRICS_ENDPOINT=metrics  # Missing leading slash
```

**Correct:**

```bash
export GOMODEL_METRICS_ENDPOINT=/metrics  # Has leading slash
```
