...
`GEMINI_BASE_URL` configures the OpenAI-compatible Gemini base URL. GoModel also
uses that compatibility surface for Gemini embeddings, files, and batches.
...
<Note>
  If you configure a custom `GEMINI_BASE_URL`, GoModel disables native Gemini
  chat routing for that provider and sends chat requests to the configured
