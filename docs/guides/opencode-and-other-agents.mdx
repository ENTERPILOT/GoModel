...
#### Thinking effort in OpenCode

OpenCode's thinking-effort switcher (the `variant_cycle` keybind) is driven by
each model's entry in its own registry. A custom provider such as `gomodel` has
no registry entry, so its models start with no variants and the switcher has
nothing to offer — nothing is missing on the GoModel side. Declare the variants
you want on the model:
...
```json
{
  ...
  "provider": {
    "gomodel": {
      ...
      "models": {
        "claude-opus-4-5": {
          "name": "Claude Opus 4.5 (via GoModel)",
          "reasoning": true,
          "variants": {
...
Each variant sends `reasoning_effort` as a top-level string on
`/v1/chat/completions`. GoModel accepts that spelling alongside its own
`"reasoning": {"effort": "..."}` object and translates either one into whatever
the routed provider expects — Anthropic thinking budgets, Gemini
`thinkingConfig`, xAI's flat field — downgrading levels the target model does
not support. Use the same effort names GoModel documents: `low`, `medium`,
`high`, `xhigh`, `max`.

<Note>
  Declare variants only for models that actually reason. A model without
  thinking support rejects the parameter upstream, and GoModel forwards that
  error unchanged.
</Note>
...
- OpenCode model variants docs: [Models](https://opencode.ai/docs/models/)
