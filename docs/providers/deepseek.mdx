...
DeepSeek's top-level `reasoning_effort` — no client change required. Clients
that already send the flat `"reasoning_effort": "..."` string (most
OpenAI-compatible coding agents do) get the same mapping; a non-empty
`reasoning.effort` wins when a request carries both. Validate
