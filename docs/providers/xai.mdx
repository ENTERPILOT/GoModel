...
client change required. A client that already sends the flat string (most
OpenAI-compatible coding agents do) gets the same per-model mapping, so an
effort level the target model does not accept is downgraded rather than
forwarded verbatim; a non-empty `reasoning.effort` wins when a request carries
both. On the Responses API the nested shape is xAI-native and passes through
unchanged.
