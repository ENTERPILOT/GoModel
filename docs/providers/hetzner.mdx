...
Hetzner exposes no `/v1/embeddings` endpoint.
...
Hetzner enforces per-key rate limits on input tokens, output tokens, and request count.
Exceeding any limit returns HTTP 429. The exact windows changed during the experimental
period, so check the
[official inference docs](https://docs.hetzner.com/general/company-and-policy/experiments/inference/)
for the current values. Prefer conservative retry settings:
