...
`gpt-5.4-mini` leave ChatGPT-authenticated Codex on August 31, 2026 — use
...
Platform, so GoModel's model registry attaches their per-token API prices.
Usage records and dashboard totals for `chatgpt` therefore show a dollar figure
that corresponds to no actual charge.
...
<Warning>
  That override corrects what `/v1/models` advertises, but **not** what usage
  records: cost tracking resolves pricing separately and still bills at
  registry rates. Anything consuming those figures acts on them — a **budget**
  can reject `chatgpt` traffic for "spending" money the subscription never
  charges, and `cost`-based load balancing will price it as API traffic. Scope
  budgets to a [user path](/features/user-path) that excludes subscription
  traffic, or leave budgets off for it.
</Warning>
