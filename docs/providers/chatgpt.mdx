...
<Warning>
  Anything that consumes those figures acts on them: a **budget** can reject
  `chatgpt` traffic for "spending" money the subscription never charges, and
  `cost`-based load balancing will price it as API traffic. Scope budgets to a
