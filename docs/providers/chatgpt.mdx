...
not resource handles: the backend pins `store: false` and rejects
`previous_response_id`, so no chaining is possible and chat clients resend
full history as usual.

Chat parameters with no Responses equivalent are rejected with a `400` that
names the field, before any upstream call: `n` greater than 1, `logit_bias`,
`stop`, `seed`, `frequency_penalty`, `presence_penalty`, `logprobs`,
`top_logprobs`, `modalities`, `audio`, `web_search_options`, and
the deprecated `functions` / `function_call` pair. `prediction` is dropped
instead: it is a speed hint and never changes the answer.
