...
Each provider can send its upstream requests through its own forward proxy.
Set `proxy_url` on the provider (or `<PROVIDER>[_SUFFIX]_PROXY_URL` as an env
...
providers:
  openai:
    ...
    proxy_url: "socks5://user:password@proxy.internal:1080"
...
```bash
export OPENAI_PROXY_URL="http://proxy.internal:3128"
export OPENAI_EU_PROXY_URL="socks5h://eu-proxy.internal:1080"
```

The default is no proxy: providers connect directly, or through whatever
