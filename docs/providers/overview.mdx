...
| Eden AI | `EDENAI_API_KEY` (`EDENAI_BASE_URL` optional) | `openai/gpt-4` | ✅ | ✅ | ✅ | ❌ | ❌ | ✅ | [Eden AI](/providers/edenai) |
...
- **Eden AI** — model IDs use `provider/model` (for example, `openai/gpt-4`)
  and are forwarded unchanged; select Eden explicitly with
  `edenai/openai/gpt-4` when another configured provider exposes the same raw
  ID. GoModel discovers Eden's catalog, context windows, capabilities, and
  per-token pricing from Eden's own `/v3/models`, so there is no built-in
  model list and new Eden models need no upgrade. Per-request cost comes from
  the exact USD `cost` Eden returns on each response rather than from token
  math. GoModel serves `/v1/responses` by translating it to Eden's
  chat-completions endpoint — Eden's own `/v3/responses` is a different API
  and is never used. See the [Eden AI guide](/providers/edenai).
