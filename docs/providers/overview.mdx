...
- **OpenCode Go (OpenCode Zen)** — routes per model: most models use
  ...
  `qwen3.7-max`, override with `GOMODEL_OPENCODE_GO_MESSAGES_MODELS`) are sent to the
...
- **Configured model lists** — available for every provider with
  ...
  `GOMODEL_CONFIGURED_PROVIDER_MODELS_MODE=fallback` uses those lists only when upstream
  `/models` is unavailable or empty. Set
  `GOMODEL_CONFIGURED_PROVIDER_MODELS_MODE=allowlist` to expose only configured models
