...
## Add providers from the dashboard

Start GoModel without provider configuration, then open the
**[Providers](http://localhost:8080/admin/dashboard/providers-config)** page
and add credentials from the UI. Every credential in the table below can be
stored this way, and the provider registers immediately without a restart.
The model catalog starts empty until you add a provider, but the API and
...
| Provider | Optional environment config | Example Model | Chat | `/responses` | Embed | Files | Batches | Passthru | Guide |
| -------- | ---------- | ------------- | :--: | :----------: | :---: | :---: | :-----: | :------: | ----- |
