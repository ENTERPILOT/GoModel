...
- **Model categories (chat vs embeddings vs audio)** — a model's category comes
  from its `modes` in the remote model registry. Local models the registry does
  not know (llama.cpp, LM Studio, Ollama, vLLM custom IDs) fall back to a
  conservative name check: IDs containing `embed` or matching well-known
  embedding families (`bge`, `e5`, `gte`, `minilm`) are categorized as
  embedding models automatically, and IDs containing `rerank` as reranking
  models. For anything the heuristic misses, declare
  `modes: [embedding]` under `providers.<name>.models` metadata in
  `config.yaml` — operator metadata always wins. Categories affect dashboard
  grouping and failover suggestions only; `/v1/embeddings` routes to any model
  the provider serves regardless of category.
