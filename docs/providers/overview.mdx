...
- **Breaker State** — the provider's live circuit-breaker state
  (see [Resilience](/advanced/resilience)).
...
send (background discovery probes are excluded), so an idle provider simply
shows no recent requests. Counters are in-memory per gateway instance and
reset on restart.
