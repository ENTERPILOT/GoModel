...
- **Image generation and audio endpoints** — llama.cpp has neither: no
  `/v1/images/generations`, `/v1/audio/speech`, or `/v1/audio/transcriptions`
  (text-to-image lives in the separate stable-diffusion.cpp project, and
  whisper.cpp's server is not OpenAI-compatible). Requests for those
  capabilities must route to a provider that serves them.
