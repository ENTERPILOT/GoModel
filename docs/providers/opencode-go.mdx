...
over an explicit `reasoning.effort` or a top-level `reasoning_effort`. Both
spellings are mapped from GoModel's recognized effort levels to their nearest
equivalent in the low / high / max set OpenCode Zen accepts:
