...
<Note>
  ...
  without it those models run without thinking, though Claude still applies its
  server-side default effort of `high`. Effort is a behavioral signal for depth
  and token spend (text, tool calls, and thinking), not a hard budget — actual
  usage varies per request and is bounded by `max_tokens`.
</Note>
