...
`MiniMax-M3` ships its chain of thought as inline `<think>` XML inside the answer content; the upstream MiniMax API describes the same shape for the M2 line (M2, M2.5, M2.7). GoModel strips those tags and moves the inner text to the canonical `reasoning_content` member of the chat completion message (and the matching `delta.reasoning_content` on streaming deltas). Recognized reasoning boundary markers are removed from answer content; only stray text that does not form a complete marker can remain.
