...
`MiniMax-M3` ships its chain of thought as inline `<think>` XML inside the answer content; the upstream MiniMax API describes the same shape for the M2 line (M2.7 and later). GoModel strips those tags and moves the inner text to the canonical `reasoning_content` member of the chat completion message (and the matching `delta.reasoning_content` on streaming deltas). Recognized reasoning boundary markers are removed from answer content; only stray text that does not form a complete marker can remain.

The step runs only for models on an exact, case-insensitive allowlist: `minimax-m2`, `minimax-m2.5`, `minimax-m2.7`, `minimax-m3`. Other MiniMax models (speech, image, embedding) and unknown future names pass through unchanged. MiniMax occasionally uses the namespaced `<mm:think>` / `<minimax:think>` marker spellings alongside the plain `<think>` ones; every spelling is accepted and stripped. An unfinished think block — the model is cut off mid-reasoning by `finish_reason: length` — is closed and the partial inner text is emitted as reasoning rather than dropped. A stray closing marker in answer content is escaped (e.g. `\<\/think\>`) so it renders as visible text and the model keeps its reasoning trace. Reasoning tokens stay completion tokens.
