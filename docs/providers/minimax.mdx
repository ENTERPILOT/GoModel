...
`MiniMax-M3` and `MiniMax-M2` ship their chain of thought as inline
`<think>...</think>` XML inside the answer content. GoModel strips those tags
and moves the inner text to the canonical `reasoning_content` member of the
chat completion message (and the matching `delta.reasoning_content` on
streaming deltas). The XML never reaches the client, so an OpenAI-compatible
caller that reads `reasoning_content` — Kimi Code, Open WebUI, anything
Anthropic-shaped — sees the reasoning as a separate field instead of as the
answer.

The step only runs for models whose name starts with `MiniMax-M3` or
`MiniMax-M2`. Other MiniMax models do not emit think tags, so non-reasoning
responses pass through byte for byte.

When the model is cut off mid-generation (`finish_reason: length`) inside an
open `<think>` block, the partial inner text is dropped. The rest of the
response still arrives verbatim. Reasoning tokens stay completion tokens;
usage accounting is unaffected.
