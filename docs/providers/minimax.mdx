...
The step runs for any model whose name starts with `MiniMax-M3` or `MiniMax-M2`. Other MiniMax models (speech, image, embedding) pass through unchanged. MiniMax occasionally closes the block with the namespaced `</mm:think>` variant; both closing spellings are accepted and stripped. An unfinished think block — the model is cut off mid-reasoning by `finish_reason: length` — is closed and the partial inner text is emitted as reasoning rather than dropped. A stray `</think>` or `</mm:think>` in answer content is escaped to `\<\/think\>` / `\<\/mm:think\>` so it renders as visible text and the model keeps its reasoning trace. Reasoning tokens stay completion tokens.
