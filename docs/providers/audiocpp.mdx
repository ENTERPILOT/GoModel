...
audio.cpp can read audio from the machine it runs on, so an upload is not
always needed. Pass an `audio` field naming a server-local path instead of a
`file` part and GoModel sends audio.cpp's JSON request shape:

```bash
curl -s http://localhost:8080/v1/audio/transcriptions \
  -H "Authorization: Bearer change-me" \
  -F model=audiocpp/moonshine-tiny \
  -F audio=/srv/audio/meeting.wav
```

This is provider-specific: every other provider still requires the upload and
...
[passthrough](/features/passthrough-api) at `/p/audiocpp/...`. The `/v1`
prefix is optional (`/p/audiocpp/audio/voices` and
