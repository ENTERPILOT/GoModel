...
- One gateway at a time, in Docker, on an AWS `c7i.large` (2 vCPU, 4 GiB), each
  ...
  `portkeyai/gateway`, `maximhq/bifrost`).
...
- It pushes the most traffic per box (`~4,200 req/s`) and the most per CPU core.
...
<Warning>
  ...
  about $0.09/hour and the run self-destructs within an hour, so budget
...
`N=20000 REPEATS=5 ./run.sh` for a heavier run, and `GOMODEL_SOURCE=../gomodel`
benchmarks a local checkout instead of the published image. The repository README
