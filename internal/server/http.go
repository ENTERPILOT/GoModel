...
type Config struct {
	...
	HeaderPolicyResolver            HeaderPolicyResolver                   // Optional: outbound header-policy resolver applied after workflow resolution
...
func New(provider core.RoutableProvider, cfg *Config) *Server {
	...
	if cfg != nil && cfg.HeaderPolicyResolver != nil {
		e.Use(HeaderPolicyPlanningMiddleware(cfg.HeaderPolicyResolver, auditLogger))
	}
