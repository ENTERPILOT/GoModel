...
// virtual_models store via role-partitioned adapters.
func NewService(store Store, catalog Catalog, defaultEnabled bool) (*Service, error) {
	...
	aliasSvc, err := aliases.NewService(redirectStore{store: store}, catalog)
	if err != nil {
		return nil, fmt.Errorf("create alias engine: %w", err)
	}
	overrideSvc, err := modeloverrides.NewService(policyStore{store: store}, catalog, defaultEnabled)
	if err != nil {
