...
// frame decoders see a complete event, and carries the envelope members
// (id, object, created, model) cached from the stream so far — never usage.
func (s *thinkStream) carryFrame(index int) []byte {
	...
	return s.choiceFrame(map[string]json.RawMessage{
		"index": encodedIndex,
...
// see a complete event. The frame inherits the envelope members (id,
// object, created, model) cached from the stream, matching how
// SynthesizeChatStream stamps the envelope on every generated chunk; when
// no chunk carried them yet they are omitted.
func (s *thinkStream) choiceFrame(choice map[string]json.RawMessage) []byte {
	chunk := make(map[string]json.RawMessage, len(s.envelope)+1)
	for key, value := range s.envelope {
		chunk[key] = value
	}
	choices := []map[string]json.RawMessage{choice}
	chunk["choices"] = mustMarshalJSON(choices)
...
func (s *thinkStream) finishSplit(raw json.RawMessage) (json.RawMessage, []byte) {
	...
	frames := append(s.carryFrame(index), s.finishFrame(index, finish)...)
	return out, frames
}
...
// frame decoders see a complete event and inherits the cached envelope
// members.
func (s *thinkStream) finishFrame(index int, finish json.RawMessage) []byte {
	// json.Marshal on an int cannot fail; the result is used directly.
	encodedIndex, _ := json.Marshal(index)
	return s.choiceFrame(map[string]json.RawMessage{
		"index":         encodedIndex,
