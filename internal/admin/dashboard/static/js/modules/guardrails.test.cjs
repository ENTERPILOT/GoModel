...
test('header row helpers manage condition and action rows in canonical shape', () => {
    const module = createGuardrailsModule();
    module.guardrailForm = { name: 'r', type: 'header_modification', description: '', user_path: '', config: {} };
    const whenField = { key: 'when', input: 'header_conditions' };
    const actionsField = { key: 'actions', input: 'header_actions' };

    module.addGuardrailHeaderRow(whenField);
    module.setGuardrailHeaderRowValue(whenField, 0, 'header', 'User-Agent');
    module.setGuardrailConditionMode(whenField, 0, 'matches');
    module.setGuardrailHeaderRowValue(whenField, 0, 'matches', '^cline/');

    assert.equal(module.guardrailConditionMode(module.guardrailHeaderRows(whenField)[0]), 'matches');
    assert.deepEqual(JSON.parse(JSON.stringify(module.guardrailForm.config.when)), [
        { header: 'User-Agent', matches: '^cline/' }
    ]);

    module.setGuardrailConditionMode(whenField, 0, 'absent');
    assert.deepEqual(JSON.parse(JSON.stringify(module.guardrailForm.config.when)), [
        { header: 'User-Agent', present: false }
    ]);

    module.addGuardrailHeaderRow(actionsField);
    module.setGuardrailHeaderRowValue(actionsField, 0, 'header', 'Anthropic-Beta');
    module.setGuardrailHeaderRowValue(actionsField, 0, 'value', 'context-1m');
    module.addGuardrailHeaderRow(actionsField);
    module.setGuardrailActionMode(actionsField, 1, 'remove');
    module.setGuardrailHeaderRowValue(actionsField, 1, 'header', 'X-Debug');

    assert.equal(module.guardrailActionMode(module.guardrailHeaderRows(actionsField)[0]), 'set');
    assert.equal(module.guardrailActionMode(module.guardrailHeaderRows(actionsField)[1]), 'remove');

    module.removeGuardrailHeaderRow(actionsField, 1);
    assert.equal(module.guardrailHeaderRows(actionsField).length, 1);
});

test('sanitizeGuardrailConfig drops incomplete header rows and empty keys', () => {
    const module = createGuardrailsModule();
    module.guardrailTypes = [{
        type: 'header_modification',
        defaults: {},
        fields: [
            { key: 'when', input: 'header_conditions' },
            { key: 'actions', input: 'header_actions' }
        ]
    }];

    const config = module.sanitizeGuardrailConfig('header_modification', {
        when: [
            { header: '  User-Agent  ', matches: '^cline/' },
            { header: 'X-Empty', equals: '' },
            { header: '', equals: 'ignored' }
        ],
        actions: [
            { action: 'set', header: 'Anthropic-Beta', value: 'context-1m', from_header: '' },
            { header: '' }
        ]
    });

    assert.deepEqual(JSON.parse(JSON.stringify(config.when)), [
        { header: 'User-Agent', matches: '^cline/' },
        { header: 'X-Empty', equals: '' }
    ]);
    assert.deepEqual(JSON.parse(JSON.stringify(config.actions)), [{ action: 'set', header: 'Anthropic-Beta', value: 'context-1m' }]);
});

test('sanitizeGuardrailConfig removes header list keys with no complete rows', () => {
    const module = createGuardrailsModule();
    module.guardrailTypes = [{
        type: 'header_modification',
        defaults: {},
        fields: [{ key: 'when', input: 'header_conditions' }]
    }];
