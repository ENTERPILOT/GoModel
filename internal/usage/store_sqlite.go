...
const (
	maxSQLiteParams      = 999
	columnsPerUsageEntry = 24
	maxEntriesPerBatch   = maxSQLiteParams / columnsPerUsageEntry // 41 entries
)
...
func (s *SQLiteStore) WriteBatch(ctx context.Context, entries []*UsageEntry) error {
	...
	for i := 0; i < len(entries); i += maxEntriesPerBatch {
		...
		query := `INSERT OR IGNORE INTO usage (id, request_id, provider_id, timestamp, model, provider, provider_name,
			endpoint, user_path, user_id, session_id, cache_type, labels, input_tokens, output_tokens, total_tokens, rewrite_tokens_saved, rewrite_cost_saved, raw_data,
