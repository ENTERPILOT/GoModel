          ...
          <div class="budget-row-period">
            {#if item.per_child}
              <span
                ...
                title="Independent counters for each direct child path"
              >
                <span>per child</span>
              </span>
            {/if}
            ...
            <span class="budget-period-label">
              <Icon
                icon={rateLimits.rateLimitIsConcurrent(item) ? Activity : Timer}
                class="budget-period-icon"
              />
        ...
        <div class="budget-bars">
          {#if item.per_child}
            <div class="per-child-summary">
              Counters are tracked independently for each direct child of <code
                >{rateLimits.rateLimitSubject(item)}</code
              >. Query <code>/v1/usage</code> as a child to inspect its counters.
            </div>
          {:else}
