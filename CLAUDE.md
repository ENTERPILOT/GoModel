...
- **Guardrails:** Definitions are persisted in the `guardrail_definitions` store and managed via the admin API/dashboard; `config/config.yaml` entries are validated and upserted into that store at startup (a seed, not the source of truth). `GUARDRAILS_ENABLED` env var gates the feature. Types: `system_prompt`, `llm_based_altering` (message-based), and `header_modification` — a workflow step that conditionally sets/removes outbound provider-request headers based on inbound headers (`when` conditions: `matches`/`equals`/`present`; `actions`: `set` with `value` or `from_header`, `remove`). Credential/transport headers are rejected at authoring time and guarded at runtime; applies on translated, passthrough, and realtime routes; each applied step is recorded as a changed-values-only delta on the audit entry's `request_revisions` chain (dashboard "Rewritten" tab).
