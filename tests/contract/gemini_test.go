...
func TestGeminiReplayResponses(t *testing.T) {
	require.True(t, goldenFileExists(t, "golden/gemini/responses.golden.json"), "missing golden file golden/gemini/responses.golden.json; run `make record-api` then `RECORD=1 go test -v -tags=contract -timeout=5m ./tests/contract/...`")
